...
# class adjust_net(nn.Module):
#     def __init__(self, out_channels=64, middle_channels=64):
#         super(adjust_net, self).__init__()
...
#             nn.Conv2d(2, middle_channels, 3, padding=1),
#             nn.ReLU(inplace=True),
#             nn.Dropout2d(p=0.1),  # 添加Dropout
#             nn.AvgPool2d(2),

#             # 第二块
#             nn.Conv2d(middle_channels, middle_channels * 2, 3, padding=1),
#             nn.ReLU(inplace=True),
#             nn.Dropout2d(p=0.2),  # 稍高的Dropout
#             nn.AvgPool2d(2),
...
#             # 第四块（新增）
#             nn.Conv2d(middle_channels * 4, middle_channels * 4, 3, padding=1),
#             nn.ReLU(inplace=True),
#             nn.Dropout2d(p=0.3),  # 更深层使用更高的Dropout
#             nn.AvgPool2d(2),
...
#         out = self.model(x)
#         out = F.adaptive_avg_pool2d(out, (1, 1))
#         out1 = out[:, :out.shape[1]//2]  # gamma
#         out2 = out[:, out.shape[1]//2:]  # beta
#         return out1, out2
