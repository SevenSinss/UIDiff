...
class Model:
    ...
    @torch.no_grad()
    def test(self):
        best_model_path = osp.join(self.logger.models_save_dir, 'ema_model-2500')
        try:
    ...
    def generate_images(self):
        ...
        for i in range(b):
            ...
            match = re.match(r'^(P\d+)_16cg_(\d+)_img\.npy$', base_filename)
            if not match:
...
logger = LoggerX(save_root=osp.join(
    osp.dirname(osp.dirname(osp.abspath(__file__))), 'output', 'corediff_16cgspect_10t'
))
...
# 处理 TIF 文件
input_folder = "../output/corediff_16cgspect_10t/save_TIF"
output_folder = "../output/corediff_16cgspect_10t/save_tif3d"
