...
GATED_DIR = os.path.join(TEMP_DIR, "gated")
STATIC_DIR = os.path.join(TEMP_DIR, "static")
GATED_SLICES_DIR = os.path.join(TEMP_DIR, "gated_slices")
STATIC_SLICES_DIR = os.path.join(TEMP_DIR, "static_slices")
NPY_DIR = os.path.join(TEMP_DIR, "npy")
OUTPUT_TIF_DIR = os.path.join(TEMP_DIR, "output_tif")
OUTPUT_TIF3D_DIR = os.path.join(TEMP_DIR, "output_tif3d")

# 创建临时文件夹
for directory in [TEMP_DIR, GATED_DIR, STATIC_DIR, GATED_SLICES_DIR, STATIC_SLICES_DIR, NPY_DIR, OUTPUT_TIF_DIR,
                  OUTPUT_TIF3D_DIR]:
...
class SpectDataLoader:
    ...
    def __getitem__(self, index):
        ...
        target = np.load(target_path)[np.newaxis, ...].astype(np.float32)
...
class Model:
    ...
    def test(self):
        ...
        for i, (low_dose, full_dose) in enumerate(tqdm.tqdm(self.test_loader, desc='测试中')):
            ...
            psnr_score, ssim_score, nmse_score = compute_measure(full_dose_norm, gen_full_dose_norm,
                                                                 data_range=data_range)
            psnr += psnr_score
            ssim += ssim_score
            nmse += nmse_score
            valid_batches += 1
            # Update progress bar for test phase (0% to 50%)
    ...
    def transfer_display_window(self, img, ref_img=None, cut_min=None, cut_max=None):
        ...
        if cut_max == cut_min:
            ...
            img = (img - (cut_min if cut_min is not None else MIN_B)) / (
                        (cut_max if cut_max is not None else MAX_B) - MIN_B)
    ...
    def generate_images(self):
        ...
        for i in range(gen_full_dose.shape[0]):
            ...
            # Update progress bar for image generation phase (50% to 95%)
            if self.progress_bar is not None:
        ...
        # Update progress bar to 100% after 3D TIF generation
        if self.progress_bar is not None:
...
with col1:
    ...
    if gated_spect_file is not None:
        file_path = os.path.join(GATED_DIR, gated_spect_file.name)
        with open(file_path, "wb") as f:
            f.write(gated_spect_file.getbuffer())
        st.success(f"门控SPECT图像 {gated_spect_file.name} 上传成功！")

    st.subheader("上传静态SPECT图像")
    static_spect_file = st.file_uploader("选择静态SPECT图像文件", type=["tif"])
    if static_spect_file is not None:
        file_path = os.path.join(STATIC_DIR, static_spect_file.name)
        with open(file_path, "wb") as f:
...
with col2:
    ...
    with col_btn1:
        if st.button("数据处理"):
            gated_files = [f for f in os.listdir(GATED_DIR) if
                           f.endswith('.tif') and os.path.isfile(os.path.join(GATED_DIR, f))]
            for file_name in gated_files:
                file_path = os.path.join(GATED_DIR, file_name)
                try:
                    img = tifffile.imread(file_path)
                    if len(img.shape) >= 3:
                        for z in range(img.shape[0]):
                            slice_path = os.path.join(GATED_SLICES_DIR, f"gated_slice_{z}.tif")
            ...
            static_files = [f for f in os.listdir(STATIC_DIR) if
                            f.endswith('.tif') and os.path.isfile(os.path.join(STATIC_DIR, f))]
            for file_name in static_files:
                ...
                try:
                    ...
                    if len(img.shape) >= 3:
                        for z in range(img.shape[0]):
                            slice_path = os.path.join(STATIC_SLICES_DIR, f"static_slice_{z}.tif")
                            tifffile.imwrite(slice_path, img[z])
                    ...
                    st.error(f"处理 {file_name} 时出错：{str(e)}")
            if not static_files:
                st.warning("未找到上传的静态SPECT TIF文件！")
    ...
    with col_btn2:
        if st.button("图像降噪"):
            try:
                ...
                model = Model(test_loader, ema_model, logger, test_images, OUTPUT_TIF_DIR, OUTPUT_TIF3D_DIR, dose,
                              diffusion_steps, progress_bar=progress_bar)
    ...
    with col_btn3:
        if st.button("清空数据"):
            if os.path.exists(TEMP_DIR):
                ...
                for directory in [TEMP_DIR, GATED_DIR, STATIC_DIR, GATED_SLICES_DIR, STATIC_SLICES_DIR, NPY_DIR,
                                  OUTPUT_TIF_DIR, OUTPUT_TIF3D_DIR]:
                    os.makedirs(directory)
